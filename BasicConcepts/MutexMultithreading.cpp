...
in C++ used to protect shared resources 
from being accessed concurrently by multiple threads. 
When multiple threads access the same data, a mutex ensures that only 
one thread can access that data at a time, 
...
By using a mutex, you ensure that shared data is protected from 
being modified simultaneously by multiple threads, 
...
The std::lock_guard<std::mutex> is used to automatically lock and 
unlock the mutex. 
...
but because std::cout is shared, the mutex ensures that only one thread 
prints at a time.
...
unlike a regular mutex that would block if the same thread tries to 
lock it again.std::timed_mutex and std::try_mutex:

These variants allow you to try to lock the mutex for a limited time 
(timed_mutex) or 
