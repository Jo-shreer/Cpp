...
A mutex mtx is declared at the global level to control access to the shared resource (in this case, std::cout).
The std::lock_guard<std::mutex> is used to automatically lock and unlock the mutex. 
When the lock_guard object goes out of scope (i.e., at the end of the function), it automatically unlocks the mutex.
The threads t1 and t2 both attempt to access std::cout, but because std::cout is shared, 
...
2. std::lock_guard:
A RAII-style (Resource Acquisition Is Initialization) lock that automatically locks the mutex when 
created and unlocks it when it goes out of scope.
Ideal for simple lock/unlock use cases.

3. std::unique_lock:
Provides more flexibility than std::lock_guard. You can manually lock and unlock the mutex using lock() and unlock().
...
unlike a regular mutex that would block if the same thread tries to lock it again.
std::timed_mutex and std::try_mutex:

These variants allow you to try to lock the mutex for a limited time (timed_mutex) or 
