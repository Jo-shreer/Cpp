...
to an object dynamically, without altering its structure. 
It allows for flexible extension of object behavior by "wrapping" an object with
additional behavior, keeping the original object's interface intact.

Key Components of the Decorator Pattern:

Component: This is an interface or abstract class defining the operations that can
            be dynamically added to an object.
ConcreteComponent: This is the actual object being wrapped. 
                   It implements the Component interface and has its base behavior.
Decorator: This abstract class implements the Component interface and has a
           reference to a Component object. The Decorator class passes the request
	   to the wrapped ConcreteComponent and may add its own behavior.
ConcreteDecorator: These are concrete classes that extend the Decorator class
                   and add additional behavior to the wrapped object.

Real-World Example:
Suppose you have a basic Window object that can be decorated with various features, 
such as scrolling, borders, or color. 
You can dynamically "decorate" this object to give it additional functionality
...
Flexible: You can add different combinations of functionality at runtime without
          creating multiple subclasses.
Single Responsibility: Each decorator class has a single responsibility,
...
Conclusion:
The Decorator Pattern provides a powerful way to dynamically add behavior to objects, 
especially when combined with other patterns.
By using decorators, you can extend functionality without changing the core object's
class, making the system more flexible and maintainable.
*/
